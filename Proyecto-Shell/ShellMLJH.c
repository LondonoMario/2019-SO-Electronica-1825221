...
int lsh_help(char **args){
	...
	printf("Cualquier comando del SO no mencionado anteriormente tambien podra ser ejecutado");
	return 1;
	}
...
int lsh_batchfile(char **args){
	if (args[1] == NULL){
		fprintf(stderr,"lsh: Se espera un argumento para \"batchfile\"\n");
	}else{
		...
		while(fgets(buff, BUFSIZE - 1, fp) != NULL) 
	    	{
	        	printf ("Contenido de la linea %d: %s\n", i ,buff);
			system(buff);
			printf("\n");
			i=i+1;
	   	}
